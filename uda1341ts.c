...
void uda1341ts_mute( uint8 on )
{
    if(on == MUTE_ON){
    	...
    	L3_putByte( (ADDRESS << 2) | DATA0, L3_ADDR_MODE  );
    	L3_putByte( (0 << 2)| (2 << 6) , L3_DATA_MODE );
    }
}
...
{
	L3_putByte( (ADDRESS << 2) | STATUS, L3_ADDR_MODE );
	L3_putByte( (1 << 7) | (converter << 0) | (1 << 5), L3_DATA_MODE );
	state+=converter;
}
...
{
	L3_putByte( (ADDRESS << 2) | STATUS, L3_ADDR_MODE );
	L3_putByte( (1 << 7) | (converter << 0), L3_DATA_MODE );
	state-=converter;
}
...
{
	L3_putByte( (ADDRESS << 2) | DATA0, L3_ADDR_MODE  );
	L3_putByte( EA | (0), L3_DATA_MODE );
	L3_putByte( ED | ~(vol), L3_DATA_MODE ); // lo ponemos negado pork a mayor atenuacion menor volumen
}
